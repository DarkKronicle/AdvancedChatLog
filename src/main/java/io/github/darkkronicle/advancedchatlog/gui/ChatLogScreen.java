...
public class ChatLogScreen extends GuiBase {
    ...
    public void add(ChatMessage message) {
        try {
            if (SearchUtils.isMatch(message.getDisplayText().getString(), search.getText(), findType)) {
    ...
    public void initGui() {
        ...
        addButton(searchType, ((button, mouseButton) -> {
            if (mouseButton == 0) {
        ...
        send = new TextFieldRunnable(2, height - 15, width - 4, 12, textRenderer, (textFieldRunnable -> {
    ...
    private void searchText(String contents) {
        ...
        for (ChatMessage m : ChatLogData.getInstance().getMessages()) {
            try {
                ...
                sorted.clear();
                FluidText text = new FluidText(RawText.withStyle(StringUtils.translate("advancedchatlog.message.regexerror"), Style.EMPTY.withColor(TextColor.fromFormatting(Formatting.RED))));
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        for (int i = scrollLine; i < scrollLine + lines; i++) {
            ...
            textRenderer.drawWithShadow(matrixStack, line.getText(), 10, height - y - 40 - fontHeight, ColorUtil.WHITE.color());
            y += lineHeight;
        }
        ScissorUtil.resetScissor();
        drawCenteredText(matrixStack, textRenderer, (scrollLine + 1) + "/" + renderLines.size(), width / 2, height - 28, ColorUtil.WHITE.color());
